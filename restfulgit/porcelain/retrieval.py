...
def get_raw_file_content(repo, tree, path):
    ...
    if git_obj.type != ObjectType.BLOB:
...
def get_contents(repo_key, repo, refspec, file_path, obj, _recursing=False):
    ...
    if not _recursing and obj.type == ObjectType.TREE:
        entries = [
    ...
    result = {
        ...
        "path": file_path,
        "size": (obj.size if obj.type == ObjectType.BLOB else 0),
        "url": contents_url,
        "git_url": git_url,
        "_links": {
            "self": contents_url,
    ...
    if not _recursing and obj.type == ObjectType.BLOB:
